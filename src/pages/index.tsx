...
export default function Home() {
	...
	const {
		...
		error,
		refetch,
	} = useQuery<Club[]>([CLUBS_ENDPOINT, router.query], ({ queryKey }) => {
		const [_, queryParams] = queryKey;
		return fetcher(CLUBS_ENDPOINT + `/?${stringify(queryParams)}`);
	});

	const { data: clubTags } = useQuery<String[]>(
		[CLUBS_ENDPOINT + "/tags"],
		() => fetcher(CLUBS_ENDPOINT + "/tags")
	);
	...
	return (
		<Layout isHome>
			<Head>
				...
				<meta
					name="description"
					content="Find a place where your heart beats. The official club directory website for the UIC Club Fair 2022"
				/>
				<link rel="icon" href="/favicon.ico" />
				<meta property="og:type" content="website" />
				<meta
					property="og:image"
					content={getUrl("/guilds-default-cover.png")}
				/>
			...
			<AspectRatio ratio={{ sm: 1330 / 760, md: 1330 / 560 }}>
				<StyledSlideShow loop>
					{clubs
						?.filter((club) => !!club.assets.cover_photo)
						.map((club) => (
					...
					>
						{(clubTags ?? []).map((tag) => (
							<Button
								...
								color="white"
								onClick={() =>
									router.push(`/?q=${tag}`, undefined, { shallow: true })
								}
...
function ClubResults({ clubs, error }: { clubs: Club[] | null; error?: any }) {
	return (
		<Flex wrap="wrap" my="10rem" width="full">
					...
					<VStack alignItems="center">
						<ModalBody textAlign="center">
							<Image
								...
								mb="4rem"
							/>
							<Text fontFamily="Space Grotesk" fontSize="50" fontWeight="bold">
								Welcome to Guilds!
							</Text>
							<Text fontSize="20">
								Guilds is the official club directory website for the UIC Club
								Fair 2022. You can search and browse all the clubs and
								organizations the university has to offer. Search your interests
								and find the best place where your heart beats!
							</Text>
						</ModalBody>

	...
	return (
		<Layout maxWidth="full" bgColor={club.theme.primary_color}>
			<Head>
				...
				<meta
					name="description"
					content={`Join ${club.name} on Guilds: the official club directory website for the UIC Club Fair 2022`}
				/>
				<meta property="og:type" content="website" />
				<meta
					property="og:image"
					content={getUrl(clubAssetURL(club, "cover_photo"))}
				/>
			...
			>
				...
				>
					...
					<Box
						...
						px={{ sm: "8", md: 0 }}
						pb={8}
						pr={club.org_type == "academic" ? { sm: 8, lg: 28 } : 0}
					>
						<Heading fontSize={{ sm: 30, lg: 40 }} mb="2">
							{club.name}
						</Heading>
						<Text fontSize={{ sm: 18, lg: 20 }}>
						...
						<BsFacebook size="30" /> */}
							{club.links.map((link, i, arr) => (
								<Fragment key={`link_top_${link._id}`}>
									...
									{i < arr.length - 1 && (
										<Box h="3" borderRight="#000 1px solid" />
									)}
								</Fragment>
							))}
						</HStack>
			...
			{club.org_type == "non-academic" && club.registration && (
				<Flex
					...
					alignItems={[
						"center",
						"center",
						club.links && club.registration ? "normal" : "center",
					]}
					px="2rem"
					py={["2rem", "2rem", "0"]}
				>
			...
			{club.links && (
				...
				>
					<Heading
						...
						mb="1.3rem"
						textAlign={
							club.registration ? { sm: "center", md: "left" } : "center"
						}
					>
